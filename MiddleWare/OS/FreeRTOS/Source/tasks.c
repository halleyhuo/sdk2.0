...
#endif /* configUSE_TICKLESS_IDLE */
				extern void goto_lowpower(void);
				//goto_lowpower();
    }
}
/*-----------------------------------------------------------*/
...
void vTaskGetRunningPlace( char * pcWriteBuffer )
{
    TaskStatus_t *pxTaskStatusArray;
    volatile UBaseType_t uxArraySize, x;
    char cStatus;
    ...
    *pcWriteBuffer = 0x00;
    ...
    if( pxTaskStatusArray != NULL )
    {
        ...
        for( x = 0; x < uxArraySize; x++ )
        {
            xTCB=(TCB_t *)pxTaskStatusArray[ x ].xHandle;
            if(xTCB!=pxCurrentTCB)
            {
                xP=(uint32_t *)xTCB->pxTopOfStack;

                sprintf( pcWriteBuffer, "%s\t 0x%x\t0x%x\n", pxTaskStatusArray[ x ].pcTaskName, pxTaskStatusArray[ x ].xHandle, *(xP+14));
...
void getCurTaskname()
{
    if(NULL != pxCurrentTCB)
        printf("Cur Task name = %s \r\n", pxCurrentTCB->pcTaskName);
    else
        printf("Cur Task name NULL \r\n");
}
