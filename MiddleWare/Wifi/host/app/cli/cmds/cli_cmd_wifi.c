...
uint8_t wifi_ps=0;//0=wakeup;1=sleep
void _set_wifi_state(uint8_t flag)
{
	wifi_ps = flag;
}

uint8_t _get_wifi_state(void)
{
	return wifi_ps;
}
...
void cmd_rc(s32 argc, char *argv[])
{
    ...
    {
        ssv6xxx_set_rate_mask((u16)ssv6xxx_atoi(argv[2]));
        LOG_PRINTF("Send cmd is OK!!\r\n");
    ...
    {
        if(strcmp(argv[2], "on") ==0){
            ssv6xxx_set_rc_resent(TRUE);
            LOG_PRINTF("resent on!!\r\n");
        }
        else{
            ssv6xxx_set_rc_resent(FALSE);
            LOG_PRINTF("resent off!!\r\n");            
        }
        return;
    }
    else if(strcmp(argv[1], "bhpwr")==0&&argc == 3)
    {
        if(strcmp(argv[2], "on") ==0){
            ssv6xxx_wifi_set_b_mode_highpwr(TRUE);
            LOG_PRINTF("b mode high power on!!\r\n");
        }
        else{
            ssv6xxx_wifi_set_b_mode_highpwr(FALSE);
            LOG_PRINTF("b mode high power off!!\r\n");            
        }
        return;
    }
    else if(strcmp(argv[1], "per")==0&&argc == 4)
    {
        //send cmd rcinfo to FW
        ssv6xxx_set_rc_per((u32)ssv6xxx_atoi(argv[2]),(u32)ssv6xxx_atoi(argv[3]));
        LOG_PRINTF("per %d/%d!!\r\n",(u32)ssv6xxx_atoi(argv[2]),(u32)ssv6xxx_atoi(argv[3]));            
        return;
    }    
    else if(strcmp(argv[1], "direct")==0&&argc == 3)
    {
        ssv6xxx_set_rc_direct_down((u32)ssv6xxx_atoi(argv[2]));
