...
int ssv6xxx_start()
{
    ...
    if(cabrio_init_mac()!= SSV6XXX_SUCCESS)
    {
    ...
    {
        if(cabrio_init_sta_mac()!=SSV6XXX_SUCCESS)
            return SSV6XXX_FAILED;
    }
    else
    {
        if(cabrio_init_ap_mac()!=SSV6XXX_SUCCESS)
            return SSV6XXX_FAILED;
    }
...
#define JOIN_DEFAULT_SSDI    "sz.test2" //"china"
#define JOIN_DEFAULT_PSK     "sz.office" //"12345678"

#define AP_DEFAULT_SSDI    "ssv-6030-AP"
...
ssv6xxx_result STAmode_default(bool bJoin)
{
    ...
    if(bJoin)
    {
        MEMCPY((void *)join_cfg.ssid.ssid,JOIN_DEFAULT_SSDI,STRLEN(JOIN_DEFAULT_SSDI));
        join_cfg.ssid.ssid_len=STRLEN(JOIN_DEFAULT_SSDI);
...
ssv6xxx_result APmode_default()
{
    ...
    mode = SSV6XXX_HWM_AP;
    ap_cfg.status = TRUE;
    MEMCPY((void *)ap_cfg.ssid.ssid,AP_DEFAULT_SSDI,STRLEN(AP_DEFAULT_SSDI));
    ap_cfg.ssid.ssid_len = STRLEN(AP_DEFAULT_SSDI);
...
int ssv6xxx_dev_init(ssv6xxx_hw_mode hmode)
{
    ...
    //rate control default value
    g_host_cfg.rate_msk = RC_DEFAULT_RATE_MSK;
    g_host_cfg.target_pf = RC_TARGET_PF;
    g_host_cfg.arith_sft = RC_ARITH_SHIFT;
    g_host_cfg.default_up_pf = RC_DEFAULT_UP_PF;
    g_host_cfg.default_down_pf = RC_DEFAULT_DOWN_PF;
    g_host_cfg.resent = RC_RESENT;
    g_host_cfg.b_mode_highpower = RC_B_MODE_HIGH_PWR;
    g_host_cfg.rc_direct_down = RC_DIRECT_DOWN;
    ...
    */
    ...
    if (1)
    {
        /* we can set default ip address and default dhcpd server config before netmgr init */
        #if 0
        netmgr_cfg cfg;
