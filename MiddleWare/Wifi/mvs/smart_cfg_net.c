...
uint8_t sconfig_flag=0,scan_falg=0,connect_wifi_flag=0;
...
int16_t smart_link_netc(char sl)
{
	uint8_t i,item, *buf;
	int32_t ret = -1;
...
//			SoundRemind(SOUND_SZWL, 1);
			...
			{
//				wifi_audio_debug("SCONFIG mode success\n");
...
//			SoundRemind(SOUND_SSWL, 1);
		...
		case CONNECT_WIFI_WAIT:
			...
			{
//				wifi_audio_debug("Connect wifi fail\n");
				vTaskDelay(1000);
				ConnectStage = CONNECT_JOIN;
				i++;
			}
